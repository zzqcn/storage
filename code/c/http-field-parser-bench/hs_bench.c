...
static hs_database_t *compileHTTPSplitDb(void)
{
    ...
    if (ret != HS_SUCCESS) {
        if (hsErr->expression < 0)
            fprintf(stderr, "compile httpSplitDb failed: %s\n", hsErr->message);
        else
            fprintf(stderr, "compile httpSplitDb failed: pattern %s with error %s\n",
...
int httpSplitCallback(unsigned int id, unsigned long long from, unsigned long long to,
